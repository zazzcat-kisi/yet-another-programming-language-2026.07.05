...
static void test_constructor_get_set_round_trips_elements(void) {
    ...
    y_packed_array_t *array =
        y_packed_array_constructor(stm, transaction, sizeof(uint32_t), 4);
    assert(array != NULL);
...
static void test_get_count_and_element_length(void) {
    ...
    /* Both are immutable, so they need no transaction. */
    size_t count = 0;
    y_packed_array_get_count(array, &count);
    assert(count == 3);
...
static void test_elements_start_zero_initialized(void) {
    ...
    y_stm_transaction_t *transaction = y_stm_begin_transaction(stm);
    y_packed_array_t *array =
        y_packed_array_constructor(stm, transaction, sizeof(uint32_t), 5);
...
static void test_set_is_isolated_from_a_concurrent_transaction(void) {
    ...
    y_stm_transaction_t *setup = y_stm_begin_transaction(stm);
    y_packed_array_t *array =
        y_packed_array_constructor(stm, setup, sizeof(uint32_t), 2);
     ...
     * the original element even after `writer` overwrites it. */
...
static void test_concurrent_setters_conflict_on_the_packed_slot(void) {
    ...
    y_stm_transaction_t *setup = y_stm_begin_transaction(stm);
    y_packed_array_t *array =
        y_packed_array_constructor(stm, setup, sizeof(uint32_t), 4);
    assert(y_stm_commit_transaction(stm, setup));
...
static void test_zero_count_array(void) {
    y_stm_t *stm = y_stm_constructor();

    y_stm_transaction_t *setup = y_stm_begin_transaction(stm);
    y_packed_array_t *array = y_packed_array_constructor(stm, setup, sizeof(uint32_t), 0);
    assert(array != NULL);
    assert(y_stm_commit_transaction(stm, setup));
    ...
    y_stm_transaction_t *transaction = y_stm_begin_transaction(stm);
    uint32_t value = 0;
    y_packed_array_get(array, transaction, 0, &value, sizeof(value)); /* out of range */
    assert(y_stm_is_rolled_back(stm, transaction));
    assert(!y_stm_commit_transaction(stm, transaction));
...
static void test_operations_on_a_null_transaction_are_safe_no_ops(void) {
    ...
    /* Immutable queries still work with no transaction. */
    size_t count = 0;
    y_packed_array_get_count(array, &count);
    assert(count == 4);
