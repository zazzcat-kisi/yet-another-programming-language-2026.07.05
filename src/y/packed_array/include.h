...
/*
 * A fixed-count, transactional array of fixed-size elements, packed
 ...
 *
 * Both the count and the element length are fixed at construction -- like
 * the blob it sits on, a packed array never grows. A class that needs a
 * resizable array layers that on top; this one is deliberately fixed, which
 * is why it fits a fixed blob so directly. Elements are zero-initialized at
 * construction.
 *
 * The bytes live in the `y_stm_t` the array was constructed with, so
 * `y_packed_array_get`/`y_packed_array_set` take a `y_stm_transaction_t *`
 * and follow y_stm's failure model (see CLAUDE.md's "Rollback on failure"):
 * a failure (out-of-range index, wrong element length, a stale snapshot, a
 * conflicting committer, ...) reports itself via y_stm_fail_transaction and
 * rolls `transaction` back rather than returning an error code. A
 ...
 * each write *any* element conflict at commit under first-committer-wins.
 * That is the trade-off of packing everything contiguously into a single
 * slot; callers wanting per-element concurrency should use separate blobs.
 */
 ...
 * `count * element_length_in_bytes` would overflow. */
...
/* The fixed element count and element length. Immutable, so unlike the
 * get/set operations these need no transaction. */
